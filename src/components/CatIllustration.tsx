...
export const CatIllustration: React.FC<CatIllustrationProps> = ({
  ...
  return (
    <div
      onClick={handleClick}
      className={`relative cursor-pointer transition-all duration-300 hover:scale-105 select-none ${className}`}
      style={{ width: showPair ? size * 1.4 : size, height: size }}
    >
      {imageSrc ? (
        /* Render Custom Story Cat Image Frame with Soft Glass Shadow */
        <div className="w-full h-full relative flex items-center justify-center">
          <div className="w-full h-full rounded-3xl overflow-hidden shadow-2xl border-4 border-white/90 bg-white/70 backdrop-blur-md p-2 hover:rotate-1 transition-transform">
            <img
              src={imageSrc}
              alt="Story Cat"
        ...
        /* Vector Animated Chibi Cat with Eye Tracking & Expressions */
        <svg viewBox="0 0 240 200" className="w-full h-full drop-shadow-xl">
          ...
          <g className={currentMood === 'celebrating' || currentMood === 'excited' ? 'animate-bounce' : ''}>
            ...
            {/* Eyes with Dynamic Mouse Tracking */}
            {isBlinking || currentMood === 'sleeping' ? (
              ...
              /* Big eyes tracking cursor pointer! */
              <g>
                <circle cx={75 + eyeOffset.x} cy={92 + eyeOffset.y} r="8" fill="#334155" />
                <circle cx={72 + eyeOffset.x} cy={89 + eyeOffset.y} r="3" fill="#FFFFFF" />
                <circle cx={115 + eyeOffset.x} cy={92 + eyeOffset.y} r="8" fill="#334155" />
                <circle cx={112 + eyeOffset.x} cy={89 + eyeOffset.y} r="3" fill="#FFFFFF" />
              </g>
            )}
